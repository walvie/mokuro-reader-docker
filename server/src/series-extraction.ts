 ...
 *
 * VENDORED COPY: kept byte-for-byte identical to src/lib/util/series-extraction.ts
 * in the frontend package (it has zero imports, so it copies cleanly). This
 * package can't reach across into $lib without the frontend's bundler/alias
 * setup, so the file is duplicated here rather than imported. If you change
 * naming/extraction behavior in the frontend copy, port the change here too.
 */
 ...
 */
function extractVolumeInfoFromName(name: string): { seriesName: string; volumeNum: string } | null {
...
function extractAnyVolumeNumber(name: string): string | null {
  ...
  const volumeInfo = extractVolumeInfoFromName(name);
 ...
 */
export function extractTitlesFromPath(path: string): { seriesTitle: string; volumeTitle: string } {
  ...
  if (isSuspectParentFolder(parentFolder) || parentContainsLeaf) {
    ...
    const volumeInfo = extractVolumeInfoFromName(leafFolder);
    if (volumeInfo) {
      return {
        seriesTitle: volumeInfo.seriesName,
        volumeTitle: `Volume ${volumeInfo.volumeNum}`
